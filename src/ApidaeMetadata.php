...
class ApidaeMetadata extends ApidaeCore
{
	...
	public function get($id, $noeud, $params = null)
	{
		...
		$result = $this->request('/api/v002/metadata/' . $id . '/' . $noeud . '/', [
			'token' => $access_token,
			'CUSTOMREQUEST' => 'GET'
		]);

		if ($result['code'] != 200) throw new ApidaeException('Retour http incorrect', ApidaeException::INVALID_HTTPCODE, [
			'debug' => $this->debug,
			'result' => $result
		]);
		return $result['body'];
